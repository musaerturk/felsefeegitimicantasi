...
import { Tool, Classroom, ExamQuestion, Student, ExamQuestion as ExamQuestionType, StudentScores } from '../../types.ts';
import Button from '../ui/Button.tsx';
import Card from '../ui/Card.tsx';
import { useClassroomManager } from '../../hooks/useClassroomManager.ts';
import { useExamAnalysisData } from '../../hooks/useExamAnalysisData.ts';
import { AcademicCapIcon, DownloadIcon, PrinterIcon } from '../ui/Icons.tsx';
...
const ExamAnalyzer: React.FC<Props> = ({ onBack }) => {
    ...
    const handleSelectClass = (classId: string) => {
        const classroom = classrooms.find((c: Classroom) => c.id === classId);
    ...
    const handleDeleteQuestion = (index: number) => {
        if (!analysisData) return;
        updateQuestions(analysisData.questions.filter((_: ExamQuestion, i: number) => i !== index));
    };
        ...
        <Card>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                    {analysisData?.questions.map((q: ExamQuestion, i: number) => (
                    ...
                    <table className="w-full text-sm text-left">
                        <thead>
                            <tr className="border-b border-gray-600">
                                ...
                                {analysisData.questions.map((q: ExamQuestion) => <th key={q.id} className="p-2 text-center" title={q.text}>{q.text} ({q.maxScore}p)</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {selectedClass.students.map((student: Student) => {
                                const studentScores = analysisData.scores.find((s: StudentScores) => s.studentId === student.id);
                                return (
                                    <tr key={student.id} className="border-b border-gray-700/50">
                                        ...
                                        {analysisData.questions.map((q: ExamQuestion) => (
    ...
    const renderReport = () => {
        ...
        const studentResults = selectedClass.students.map((student: Student) => {
            const scoresData = analysisData.scores.find((s: StudentScores) => s.studentId === student.id);
            const totalScore = analysisData.questions.reduce((sum: number, q: ExamQuestionType) => sum + (scoresData?.scores[q.id] ?? 0), 0);
            return {
                id: student.id,
                name: student.name,
                totalScore,
        ...
        const successfulStudents = studentResults.filter((s: { isSuccessful: boolean; }) => s.isSuccessful).length;
        ...
        const questionSuccessRates = analysisData.questions.map((q: ExamQuestionType) => {
            const totalPointsForQuestion = analysisData.scores.reduce((sum: number, s: StudentScores) => sum + (s.scores[q.id] ?? 0), 0);
        ...
        const scoreDistribution = scoreRanges.slice(0, -1).map((start: number, i: number) => {
            const end = scoreRanges[i+1];
            const count = studentResults.filter((s: { totalScore: number; }) => s.totalScore >= start && s.totalScore < end).length;
            return { label: `${start}-${end-1}`, count };
        });
        scoreDistribution[scoreDistribution.length - 1].label = "90-100";
        scoreDistribution[scoreDistribution.length - 1].count += studentResults.filter((s: { totalScore: number; }) => s.totalScore === 100).length;
        ...
        useEffect(() => {
            ...
            createChart('questionSuccessChart', 'bar', questionSuccessRates.map((q: {label: string}) => q.label), questionSuccessRates.map((q: {rate: number}) => q.rate), 'Soru Başarı Oranı (%)');
            ...
            createChart('scoreDistributionChart', 'bar', scoreDistribution.map((r: {label: string}) => r.label), scoreDistribution.map((r: {count: number}) => r.count), 'Puan Dağılımı (Öğrenci Sayısı)');
        ...
        const generatePdfReport = async (action: 'save' | 'print') => {
            ...
            // Temporarily apply light-theme styles for printing
            const originalClassName = reportElement.className;
            reportElement.className = 'p-4 bg-white';
            ...
            Object.values(chartsRef.current).forEach(chart => {
                ...
                if (chart.options.scales) {
                    ...
                    chart.options.scales.x.grid.color = '#ddd';
                }
                chart.update();
            });
        
            await new Promise(resolve => setTimeout(resolve, 100)); // Wait for charts to redraw
            ...
            try {
                ...
                if (action === 'save') {
                    pdf.save(`${analysisData.examTitle.replace(/ /g, '_')}_analiz.pdf`);
                } else {
                    pdf.autoPrint();
                ...
                Object.values(chartsRef.current).forEach(chart => {
                    ...
                    if (chart.options.scales) {
                        ...
                        chart.options.scales.y.grid.color = '#4b5563';
                        chart.options.scales.x.ticks.color = '#9ca3af';
                        chart.options.scales.x.grid.color = '#4b5563';
                    }
                    chart.update();
                });
                 ...
                 <div ref={reportRef} className="p-4 bg-gray-900">
                    <Card>
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {studentResults.map((s: { id: string, name: string, totalScore: number, isSuccessful: boolean }) => (
